...
let SIX_BLOCKS_PRELOOP_TAIL_CORRECT = prove
 (`!in_ptr out_ptr xi_ptr ivec_ptr key_ptr htable_ptr
  ...
  (* Cascade more_than_5 -> less_than_1: store ct1..ct6 + GHASH accumulate.
  ...
  MAP_EVERY (fun n ->
    ...
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (218--255) THEN
  ...
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC SIX_BLOCKS_PRELOOP_TAIL_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (311--318) THEN

  GCM_NBLOCK_POST_SIM_NORMALIZE_TAC THEN

  (* GHASH karatsuba reduce continues; reach PC=pc+1512 (just after final eor3
     forms v19) before abbreviating final_xi. *)
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC SIX_BLOCKS_PRELOOP_TAIL_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (319--354) THEN
  ABBREV_FINAL_XI_TAC THEN

  (* Epilogue (ext, rev64, str, mov, ldp x4): plain stepping, stop AT the ret. *)
  ARM_STEPS_TAC SIX_BLOCKS_PRELOOP_TAIL_EXEC (355--362) THEN

  CONV_TAC(TOP_DEPTH_CONV let_CONV) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN

  (* Seven-way conjunction: ct1..ct6, GHASH. *)
  CONJ_TAC THENL [
    GCM_CT1_STEP_TAC;
    CONJ_TAC THENL [
      GCM_CT2_STEP_TAC;
