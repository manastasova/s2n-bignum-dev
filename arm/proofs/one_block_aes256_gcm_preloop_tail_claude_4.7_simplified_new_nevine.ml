...
(* GCM_GHASH_STEP_TAC : nevine-flavoured closure for our spec.
   nevine.ml's "minimal" recipe — unfold + let_CONV + ABBREV_ALL_PMUL +
   WORD_BLAST — works only when Htable stores plain H, so the LHS pmul atoms
   match RHS atoms verbatim.  Our spec stores byteswap128(H) (matching
   aws-lc gcm_init_v8 — see ghashv8-armx.pl line 110 + aesv8-gcm-armv8.pl
   line 861-864), so the bridge GHASH_1BLOCK_KARATSUBA_EQ_POLYVAL_DOT is
   needed to translate between byteswap128(h) atoms and h atoms.
   This closure reuses the proven bridge-based recipe but keeps nevine's
   spirit: a single named tactic ending in ABBREV_ALL_PMUL_TAC + WORD_BLAST. *)
