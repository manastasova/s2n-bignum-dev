...
(* ========================================================================= *)
(*  GHASH STEP TACTIC (N=3 instance)                                          *)
(*                                                                           *)
(* Mirrors the 2-block style. For N=3:                                       *)
(*   - 12 atomic ABBREVs:                                                    *)
(*       uA0/uA1 (subwords of RF ct3), uB0/uB1 (subwords of RF ct2),         *)
(*       uG0/uG1 (subwords of RF ct1), uC0/uC1 (subwords of RF xi),          *)
(*       uD0/uD1 (subwords of h),                                              *)
(*       uE0/uE1 (subwords of polyval_dot h h),                                *)
(*       uF0/uF1 (subwords of polyval_dot h (polyval_dot h h))                 *)
(*   - 9 inner pmul ABBREVs (p1..p9): block-1 pl/ph/pm + block-2 + block-3   *)
(*   - 19 z-vars (z1..zJ, two per inner pmul + 1 for small pmul subword)     *)
(*   - 1 outer XOR-AC SUBGOAL_THEN (BIG pmul args)                            *)
(*   - 5 outer ABBREVs (qBigP, qSmallP, qBigPL, qBigPH, qSmallPH)             *)
(*   - BINOP_TAC THENL [WORD_BITWISE_RULE; WORD_BITWISE_RULE]                  *)
(* ========================================================================= *)
...
let GCM_3BLOCK_GHASH_STEP_TAC =
  ...
  (* Substitute the byte-level Q19 = final_xi hypothesis so the LHS becomes the
     byte-form word_join expression. REV64 lemmas then collapse the byte chunks
     back into `word_reversefields 8 _`, allowing MATCH_MP_TAC to peel both sides.
     Filter selects the hypothesis whose LHS head is `word_join` (not `read`). *)
  ...
  REWRITE_TAC[WORD_XOR_ASSOC] THEN
  (* Common-pattern abbreviations: 14 atomic subwords (3 cts + xi + 3 H powers) *)
  REWRITE_TAC[karatsuba_mid; WORD_REVERSEFIELDS_XOR_8_128; WORD_SUBWORD_XOR] THEN
  ABBREV_TAC `(uA0:(64)word) = word_subword (word_reversefields 8 (ct3:(128)word)) (0,64)` THEN
  ABBREV_TAC `(uA1:(64)word) = word_subword (word_reversefields 8 (ct3:(128)word)) (64,64)` THEN
  ABBREV_TAC `(uB0:(64)word) = word_subword (word_reversefields 8 (ct2:(128)word)) (0,64)` THEN
  ABBREV_TAC `(uB1:(64)word) = word_subword (word_reversefields 8 (ct2:(128)word)) (64,64)` THEN
  ABBREV_TAC `(uG0:(64)word) = word_subword (word_reversefields 8 (ct1:(128)word)) (0,64)` THEN
  ABBREV_TAC `(uG1:(64)word) = word_subword (word_reversefields 8 (ct1:(128)word)) (64,64)` THEN
  ...
  ASM_REWRITE_TAC[] THEN
  REWRITE_TAC[WORD_XOR_ASSOC] THEN
  (* Normalize XOR-AC of small pmul args BEFORE inner pmul ABBREVs so that
     the inner pmul ABBREV LHS shapes (uG0⊕uC0, uG1⊕uC1, uA0⊕uA1, uB0⊕uB1)
     actually match what is in the goal. Mirrors the 2-block file's order. *)
  SUBGOAL_THEN
    `(word_xor uC0 uG0 = word_xor uG0 uC0:(64)word) /\
     (word_xor uC1 uG1 = word_xor uG1 uC1:(64)word) /\
     (word_xor uA1 uA0 = word_xor uA0 uA1:(64)word) /\
     (word_xor uB1 uB0 = word_xor uB0 uB1:(64)word) /\
     (word_xor uC1 (word_xor uG1 (word_xor uC0 uG0)) =
        word_xor (word_xor uG0 uC0) (word_xor uG1 uC1):(64)word) /\
     (word_xor uC0 (word_xor uG0 (word_xor uC1 uG1)) =
        word_xor (word_xor uG0 uC0) (word_xor uG1 uC1):(64)word)`
    (fun th -> REWRITE_TAC[th]) THENL
    [REPEAT CONJ_TAC THEN CONV_TAC WORD_RULE; ALL_TAC] THEN
  (* 9 inner pmul ABBREVs (3 per block × 3 blocks) — mirrors 2-block's p1-p6
     scaled to N=3. Naming: p<block>_<lo|hi|mid> where block ∈ {1,2,3}.
     Block 1 (xi⊕ct1, h³): uG⊕uC inputs, uF H-power.
     Block 2 (ct2, h²):   uB inputs, uE H-power.
     Block 3 (ct3, h):    uA inputs, uD H-power. *)
  ABBREV_TAC `(p1_lo:(128)word) =
    word_pmul (word_xor (uG0:(64)word) (uC0:(64)word)) (uF0:(64)word)` THEN
  ABBREV_TAC `(p1_hi:(128)word) =
    word_pmul (word_xor (uG1:(64)word) (uC1:(64)word)) (uF1:(64)word)` THEN
  ABBREV_TAC `(p1_mid:(128)word) =
    word_pmul (word_xor (word_xor (uG0:(64)word) (uC0:(64)word))
                       (word_xor (uG1:(64)word) (uC1:(64)word)))
              (word_xor (uF0:(64)word) (uF1:(64)word))` THEN
  ABBREV_TAC `(p2_lo:(128)word) = word_pmul (uB0:(64)word) (uE0:(64)word)` THEN
  ABBREV_TAC `(p2_hi:(128)word) = word_pmul (uB1:(64)word) (uE1:(64)word)` THEN
  ABBREV_TAC `(p2_mid:(128)word) =
    word_pmul (word_xor (uB0:(64)word) (uB1:(64)word))
              (word_xor (uE0:(64)word) (uE1:(64)word))` THEN
  ABBREV_TAC `(p3_lo:(128)word) = word_pmul (uA0:(64)word) (uD0:(64)word)` THEN
  ABBREV_TAC `(p3_hi:(128)word) = word_pmul (uA1:(64)word) (uD1:(64)word)` THEN
  ...
  (* 18 z-vars for lo/hi subwords of 9 inner pmuls. Mirrors 2-block's
     z1..zC for 6 pmuls; here scaled to N=3 (3 blocks × 3 pmuls each). *)
  ...
  ASM_REWRITE_TAC[] THEN
  REWRITE_TAC[WORD_XOR_ASSOC] THEN
  (* Normalize the small pmul on the RHS to share the (z1_lo, z4_lo, z7_lo)
     form used by zD and qSmallP. Without this, the RHS small_pmul argument
     reads (z7_lo, z4_lo, z1_lo) and would not be folded by zD/qSmallP. *)
  SUBGOAL_THEN
    `word_pmul (word_xor (z7_lo:(64)word) (word_xor z4_lo z1_lo))
               (word 13979173243358019584:(64)word):(128)word =
     word_pmul (word_xor (z1_lo:(64)word) (word_xor z4_lo z7_lo))
               (word 13979173243358019584:(64)word):(128)word`
    (fun th -> REWRITE_TAC[th]) THENL
    [AP_THM_TAC THEN AP_TERM_TAC THEN CONV_TAC WORD_RULE; ALL_TAC] THEN
  (* zD: subword of small inner pmul (Barrett w · low-half accumulator).
     The accumulator's lo half is z1_lo⊕z4_lo⊕z7_lo (the three blocks' lo*lo).
     Use the (z1_lo, z4_lo, z7_lo) ordering that actually appears in the goal
     after WORD_XOR_ASSOC (matches both top-level and nested-inside-BIG-pmul). *)
  ABBREV_TAC `(zD:(64)word) =
    word_subword (word_pmul
      (word_xor (z1_lo:(64)word) (word_xor (z4_lo:(64)word) (z7_lo:(64)word)))
      (word 13979173243358019584:(64)word):(128)word) (0,64)` THEN
  ASM_REWRITE_TAC[] THEN
  REWRITE_TAC[WORD_XOR_ASSOC] THEN
  ASM_REWRITE_TAC[] THEN
  (* Normalize the BIG outer pmul arg (XOR-AC of all 13 z-atoms). LHS uses
     (z9_lo,z6_lo,z3_lo,z7_lo,z4_lo,z1_lo,z8_lo,z5_lo,z2_lo,zD,z7_hi,z4_hi,z1_hi);
     RHS uses (z1_hi,z4_hi,z7_hi,z3_lo,z6_lo,z9_lo,z2_lo,z5_lo,z8_lo,z1_lo,z4_lo,z7_lo,zD).
     They are XOR-AC equivalent; WORD_RULE proves it. *)
  SUBGOAL_THEN
    `word_pmul
       (word_xor (z9_lo:(64)word)
        (word_xor z6_lo
        (word_xor z3_lo
        (word_xor z7_lo
        (word_xor z4_lo
        ...
        (word_xor z2_lo
        (word_xor zD
        (word_xor z7_hi (word_xor z4_hi z1_hi))))))))))))
       (word 13979173243358019584:(64)word):(128)word =
     word_pmul
       (word_xor (z1_hi:(64)word)
        ...
        (word_xor z8_lo
        (word_xor z1_lo
        (word_xor z4_lo (word_xor z7_lo zD))))))))))))
       (word 13979173243358019584:(64)word):(128)word`
    ASSUME_TAC THENL
    [AP_THM_TAC THEN AP_TERM_TAC THEN CONV_TAC WORD_RULE; ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  (* 5 outer ABBREVs (qBigP, qSmallP, qBigPL, qBigPH, qSmallPH) reduce the
     remaining big terms to opaque atoms, after which BINOP+WORD_RULE closes. *)
  ABBREV_TAC
    `qBigP = word_pmul
       (word_xor (z1_hi:(64)word)
        (word_xor z4_hi
  ...
  ABBREV_TAC
    `qSmallP = word_pmul
       (word_xor (z1_lo:(64)word) (word_xor z4_lo z7_lo))
       (word 13979173243358019584:(64)word):(128)word` THEN
  ...
  (* Each half has 16-17 atoms; WORD_RULE times out, but WORD_BITWISE_RULE
     handles XOR-AC at this scale efficiently. *)
  BINOP_TAC THENL [CONV_TAC WORD_BITWISE_RULE; CONV_TAC WORD_BITWISE_RULE];;
...
(* The GHASH closure follows the 2-block recipe scaled to 14 atomic         *)
(* ABBREVs + 9 inner pmul ABBREVs + 18 z-vars + zD + 5 outer ABBREVs        *)
(* + BINOP_TAC + WORD_RULE. NO CHEAT_TAC; NO axioms.                         *)
(* ========================================================================= *)
