...
(*   - GCM_6BLOCK_GHASH_STEP_TAC + main theorem AES256_GCM_SIX_BLOCK_CORRECT *)
(* ========================================================================= *)
...
(* ========================================================================= *)
(*  PER-N: 6-block assembly-shape spec ghash_6block_karatsuba.               *)
(* ========================================================================= *)
...
let GCM_CT1_STEP_TAC = GCM_NBLOCK_CT_STEP_TAC 6 1;;
let GCM_CT2_STEP_TAC = GCM_NBLOCK_CT_STEP_TAC 6 2;;
...
(* ========================================================================= *)
(*  PARTIAL-FINAL-BLOCK GHASH CLOSER (masked ct6) + cascade/mask helpers.     *)
(* ========================================================================= *)
...
let SIXBLOCK_GT_COND = prove
 (`!byte_len t. byte_len <= 16 /\ t <= 96 ==>
    ((~(val (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word t)) = 0) /\
      (ival (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word t)) < &0 <=>
       ~(ival (word_ushr (word (640 + 8 * byte_len):int64) 3) - &t =
         ival (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word t)))))
     <=> t < 80 + byte_len)`,
  REPEAT STRIP_TAC THEN
  ...
  ASM_SIMP_TAC[IVAL_WORD_SUB_SMALL; NBLOCK_IVAL_WORD_SMALL;
               ARITH_RULE `byte_len <= 16 ==> 80 + byte_len < 2 EXP 63`;
               ARITH_RULE `t <= 96 ==> t < 2 EXP 63`] THEN
  REWRITE_TAC[GSYM INT_OF_NUM_ADD; GSYM INT_OF_NUM_LT] THEN
  ASM_INT_ARITH_TAC);;

let SIXBLOCK_GT_COND_FALSE = prove
 (`!byte_len t. byte_len <= 16 /\ 96 <= t /\ t <= 96 ==>
    ((~(val (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word t)) = 0) /\
      (ival (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word t)) < &0 <=>
       ~(ival (word_ushr (word (640 + 8 * byte_len):int64) 3) - &t =
         ival (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word t)))))
     <=> F)`,
  REPEAT STRIP_TAC THEN
  ASM_SIMP_TAC[SIXBLOCK_GT_COND; ARITH_RULE `96 <= t /\ t <= 96 ==> t <= 96`] THEN
  ASM_ARITH_TAC);;

let SIXBLOCK_GT_COND_TRUE = prove
 (`!byte_len. 1 <= byte_len /\ byte_len <= 16 ==>
    ((~(val (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word 80)) = 0) /\
      (ival (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word 80)) < &0 <=>
       ~(ival (word_ushr (word (640 + 8 * byte_len):int64) 3) - &80 =
         ival (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word 80)))))
     <=> T)`,
  REPEAT STRIP_TAC THEN
  ASM_SIMP_TAC[SIXBLOCK_GT_COND; ARITH_RULE `80 <= 96`] THEN
  ASM_ARITH_TAC);;

let SIXBLOCK_CASC96 = prove
 (`!byte_len. byte_len <= 16 ==>
    ((~(val (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word 96)) = 0) /\
      (ival (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word 96)) < &0 <=>
       ~(ival (word_ushr (word (640 + 8 * byte_len):int64) 3) - &96 =
         ival (word_sub (word_ushr (word (640 + 8 * byte_len):int64) 3) (word 96))))) <=> F)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SIXBLOCK_GT_COND_FALSE THEN ASM_ARITH_TAC);;
...
let SIXBLOCK_CASCADE_TAC : tactic =
  FIRST_X_ASSUM(fun bl16 -> if concl bl16 = `byte_len <= 16` then
    FIRST_X_ASSUM(fun bl1 -> if concl bl1 = `1 <= byte_len` then
      RULE_ASSUM_TAC(REWRITE_RULE[
        MATCH_MP SIXBLOCK_CASC96 bl16;
       
        MATCH_MP SIXBLOCK_GT_COND_TRUE (CONJ bl1 bl16); COND_CLAUSES]) THEN
...
let AES256_GCM_SIX_BLOCK_CORRECT = prove
 (`!in_ptr out_ptr xi_ptr ivec_ptr key_ptr htable_ptr
    ...
    aligned 16 stackptr /\
    nonoverlapping (word pc,1600) (in_ptr:int64,96) /\
    nonoverlapping (word pc,1600) (out_ptr:int64,96) /\
    nonoverlapping (word pc,1600) (xi_ptr:int64,16) /\
    nonoverlapping (word pc,1600) (ivec_ptr:int64,16) /\
    nonoverlapping (word pc,1600) (key_ptr:int64,240) /\
    nonoverlapping (word pc,1600) (htable_ptr:int64,256) /\
    nonoverlapping (word pc,1600) (stackptr:int64,80) /\
    ...
    nonoverlapping (ivec_ptr,16) (word pc,1600) /\
  ...
  REWRITE_TAC[C_ARGUMENTS; MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI;
              ...
              fst AES256_GCM_SIX_BLOCK_EXEC] THEN
  REPEAT STRIP_TAC THEN ENSURES_INIT_TAC "s0" THEN

  (* Prologue *)
  ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC (1--19) THEN
  ...
  (* AES rounds for all 6 blocks, up to just after the block-result eor3s
     (PC = pc+868), BEFORE the dispatch register-shuffle cascade. *)
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC [n] THEN
    GCM_ENC_SIMPLIFY_TAC) (20--217) THEN

  (* Abbreviate s13_1..s13_6 from Q0..Q5 (clean sequential pre-shuffle order). *)
  FIRST_ASSUM(fun th ->
  ...
  (* Tail-dispatch cascade (more_than_5) + store ct1..ct6 + GHASH accumulate.
     With symbolic byte_len each b.gt leaves the PC as a conditional; resolve
     after every step (fall-through at 96 then the taken branch at 80).
     Restore 2 EXP 64 after each step (the stores need it). *)
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`]) THEN
    SIXBLOCK_CASCADE_TAC) (218--255) THEN
  ABBREV_TAC `ct2 = word_xor (word_xor pt2 s13_2) rk14:(128)word` THEN
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (256--262) THEN
  ABBREV_TAC `ct3 = word_xor (word_xor pt3 s13_3) rk14:(128)word` THEN
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (263--283) THEN
  ABBREV_TAC `ct4 = word_xor (word_xor pt4 s13_4) rk14:(128)word` THEN
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (284--295) THEN
  ABBREV_TAC `ct5 = word_xor (word_xor pt5 s13_5) rk14:(128)word` THEN
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (296--310) THEN
  ABBREV_TAC `ct6 = word_xor (word_xor pt6 s13_6) rk14:(128)word` THEN

  (* Steps 311-332: finish building the partial-block mask register (Q0). *)
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (311--332) THEN
  ...
  (* Steps 333-355: AND_VEC, bif (partial store fixup), GHASH karatsuba +
     Barrett reduction (up to the final EOR3 in Q19). *)
  MAP_EVERY (fun n ->
    ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC [n] THEN GCM_ENC_SIMPLIFY_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `18446744073709551616 = 2 EXP 64`])) (333--355) THEN
  ...
  (* Epilogue (ext, rev64, str, mov, ldp x4): plain stepping.  Stop at s363
     (PC = pc+1548, just before the RET at pc+1548). *)
  ARM_STEPS_TAC AES256_GCM_SIX_BLOCK_EXEC (356--363) THEN

  CONV_TAC(TOP_DEPTH_CONV let_CONV) THEN
  ENSURES_FINAL_STATE_TAC THEN
  (* Collapse the mask register that survives into the ct6 store goal, and
     reduce X0 = word(80+byte_len). *)
  SUBGOAL_THEN `1 <= byte_len /\ byte_len <= 16` MP_TAC THENL
   [ASM_REWRITE_TAC[]; ALL_TAC] THEN
  ...
  (* After ASM_SIMP discharges the PC and X0 conjuncts the remaining goals are
     the ct1..ct5 stores, the masked ct6 store, and the GHASH over
     [ct1; ct2; ct3; ct4; ct5; ctm6]. *)
  REPEAT CONJ_TAC THENL [
    ...
    GCM_6BLOCK_GHASH_STEP_MASKED_TAC
  ]);;
