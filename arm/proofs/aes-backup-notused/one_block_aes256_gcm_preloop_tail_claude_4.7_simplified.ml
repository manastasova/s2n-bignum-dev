(* ========================================================================= *)
(* Correctness proof for one_block_aes256_gcm_preloop_tail (SIMPLIFIED)      *)
(*                                                                           *)
(* Cleanup of one_block_aes256_gcm_preloop_tail_claude_4.7.ml.               *)
(* Each change below was verified by reloading the full proof                *)
(* (~12 min simulation) and confirming the top theorem still closes.         *)
(*                                                                           *)
(* Lemma-level: hand-rolled replaced by library equivalents                   *)
(*   WORD_OR_SELF                       -> WORD_OR_REFL                      *)
(*   WORD_AND_ONES_128                  -> WORD_AND_NOT0                     *)
(*   local `let WORD_XOR_ASSOC = ...`   -> library WORD_XOR_ASSOC            *)
(*   ad-hoc `WORD_RULE word_sub 80`     -> general STACK_PTR_CANCEL          *)
(*   ad-hoc `WORD_RULE word_add (n+m)`  -> library WORD_ADD_ASSOC_CONSTS     *)
(*                                                                           *)
(* Lemma-level: deletions and consolidations                                  *)
(*   BIF_ALLONES intermediate step      -> deleted, BIF_MASK via WORD_BLAST  *)
(*   `WORD_BITWISE_RULE word_xor (word 0) x = x` inlined twice               *)
(*                                       -> single WORD_XOR_0_LEFT lemma     *)
(*                                                                           *)
(* Tactic-level: redundant passes removed (search for "SIMPLIFIED-REMOVED")   *)
(*   #5: redundant SIMD_SIMPLIFY_ASSUM_TAC + WORD_SWAP_HALVES_INVOLUTION     *)
(*       (both already done inside per-step GCM_ENC_SIMPLIFY_TAC)            *)
(*   #5: redundant WORD_SIMPLE_SUBWORD_CONV pass after steps 85--93           *)
(*   #6: redundant PMUL_NORM_CONV pass before the 94--111 simulation loop   *)
(*   #7: two back-to-back XOR-simp REWRITE_TACs merged into one              *)
(*                                                                           *)
(* Retained redundancies                                                      *)
(*   The 5 back-to-back CONV_TAC(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)    *)
(*   calls in the GHASH closure each look load-bearing because the           *)
(*   intervening REWRITE_TACs (WORD_INSERT_AS_JOIN_1/2, KAR_SUBWORD_LEMMA,   *)
(*   HALFSWAP_XOR, REV8_JOIN_FOLD, KAR_MID_BRIDGE, GSYM WORD_SUBWORD_XOR)    *)
(*   all introduce fresh word_subword(word_join ...) shapes.                 *)
(* ========================================================================= *)
...
(* NOTE: s2n-bignum's base.ml (transitively) loads WORD_XOR_ASSOC in the form
     |- word_xor (word_xor a b) c = word_xor a (word_xor b c)
   (right-associate / flattening).  That is the direction this proof wants
   everywhere, so we simply use the library theorem directly — no local
   redefinition needed (the original file did redefine it identically). *)
...
(* Library-backed where possible: WORD_OR_REFL, WORD_AND_NOT0 replace
   earlier hand-written WORD_OR_SELF, WORD_AND_ONES_128.                     *)
...
let WORD_AND_MASK_SYM = prove(
  ...
  REWRITE_TAC[MASK_IS_ONES; WORD_AND_NOT0]);;
(* BIF_ALLONES deleted: WORD_BLAST proves the full BIF_MASK directly. *)
let BIF_MASK = prove(
...
(* Stack/pointer arithmetic normalizer: use library WORD_ADD_ASSOC_CONSTS and
   a fully-general subtract-add cancellation (no hard-coded 80). *)
let STACK_PTR_CANCEL = WORD_RULE
  `!(x:(N)word) y. word_sub (word_add x y) y = x`;;

(* ---- Per-step cleanup (identical to original's GCM_ENC_SIMPLIFY_TAC) ----- *)
...
(* Library WORD_XOR_0 only handles the right-hand 0 form:
       |- word_xor x (word 0) = x
   The left-hand form also appears during closure and needs a matching rule.
   Keep this as a single named lemma so the main proof doesn't pay the cost
   of re-running WORD_BITWISE_RULE at each use site. *)
...
(* Uses variant to get fresh names that don't collide with prior abbreviations,
   so multiple passes of the tactic generate unique names like pm0, pm0', pm0''. *)
...
(* Double subword through word_join: collapse swap-halves followed by lower-half. *)
...
let ONE_BLOCK_PRELOOP_TAIL_CORRECT = prove
 (`!in_ptr out_ptr xi_ptr ivec_ptr key_ptr htable_ptr
  ...
  (* Steps 1-19: prologue.
     Stack-arithmetic normalisation uses the general library lemmas
     WORD_ADD_ASSOC_CONSTS + STACK_PTR_CANCEL (no hard-coded 80). *)
  ...
  (* [SIMPLIFIED-REMOVED #5] WORD_SIMPLE_SUBWORD_CONV pass was redundant
     with GCM_ENC_SIMPLIFY_TAC's own pass at step 93. *)
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_SWAP_HALVES_INVOLUTION;
  ...
  (* [SIMPLIFIED-REMOVED #6] Pre-94 PMUL_NORM_CONV pass.  Each of steps
     94--111 re-runs GCM_ENC_SIMPLIFY_TAC which re-creates pmul hypotheses
     in original (unnormalized) argument order anyway, so this up-front
     pass is wasted work. *)

  (* Steps 94-111: GHASH Karatsuba + reduction + restores.  Step 105 is
     the ~3.5 min bottleneck (arm_REV64_VEC Q19); see the original file
  ...
  (* [SIMPLIFIED-REMOVED #5] Redundant SIMD_SIMPLIFY_ASSUM_TAC +
     WORD_SWAP_HALVES_INVOLUTION pass.  Both are already applied inside
     GCM_ENC_SIMPLIFY_TAC (which was run after every step 94--111). *)
  RULE_ASSUM_TAC(fun th ->
    try CONV_RULE(RAND_CONV(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)) th
    with _ -> th) THEN
  ...
  CONJ_TAC THENL [
    ...
    (* GHASH subgoal: bridge to ghash_1block_karatsuba, then match assembly shape *)
    ...
    CONV_TAC(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV) THEN
    CONV_TAC(TOP_DEPTH_CONV PMUL_NORM_CONV) THEN
    (* [SIMPLIFIED-REMOVED #7] Redundant REWRITE_TAC for WORD_XOR_ASSOC +
       WORD_XOR_0 + WORD_XOR_0_LEFT merged into the next REWRITE_TAC. *)
    ...
    CONV_TAC(TOP_DEPTH_CONV PMUL_NORM_CONV) THEN
    REWRITE_TAC[WORD_XOR_ASSOC] THEN
    (* Abbreviate inner word_pmul, fold WORD_SUBWORD_XOR, sort XOR args,
       abbreviate the resulting outer word_pmul, then close *)
