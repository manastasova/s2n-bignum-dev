(* ========================================================================= *)
(* Correctness proof for one_block_aes256_gcm_preloop_tail                   *)
(* Postcondition uses ghash_polyval_acc (composable GHASH spec).             *)
(*                                                                           *)
(* Structure:                                                                *)
(*  1. ghash_1block_karatsuba: assembly-shaped intermediate spec              *)
(*  2. GHASH_1BLOCK_KARATSUBA_EQ_POLYVAL_DOT: algebraic bridge               *)
(*  3. ARM simulation proof                                                  *)
(*  4. Closure: reduce stored tag to ghash_polyval_acc form                  *)
(* ========================================================================= *)
...
let ONE_BLOCK_PRELOOP_TAIL_CORRECT = prove
 (`!in_ptr out_ptr xi_ptr ivec_ptr key_ptr htable_ptr
  ...
  (* Steps 94-111: GHASH Karatsuba + reduction + restores.
     ...
     TRADEOFF NOTE on step 105 (arm_REV64_VEC Q19 Q19 8, the slowest single
     step at ~3.5 min): Q19 at that point holds the full Karatsuba+Barrett
     expression, and the byte-level REV64 unfolding explodes the term size
     before GCM_ENC_SIMPLIFY_TAC refolds it via REV64_*_LANE lemmas.

     Abbreviating Q19 before step 105 (ABBREV_TAC `q19_s104 = <Q19 at s104>`)
     cuts step 105 to ~3s and the overall simulation to ~25s.  However, this
     interferes with a cancellation in step 106 (the STR that consumes Q19):
     without the abbreviation, GCM_ENC_SIMPLIFY_TAC collapses the half-swap
     pattern `word_subword (word_join X X) (64,128)` that arises on the
     stored path; with the abbreviation the pattern survives and leaves an
     extra layer that breaks the downstream closure tactic.

     We therefore accept the slow step 105 here.  A cleaner speedup would
     require a more surgical abbreviate/unfold around just REV64_VEC, which
  ...
  CONJ_TAC THENL [
    ...
    REWRITE_TAC[WORD_XOR_ASSOC; KAR_MID_BRIDGE; WORD_SUBWORD_0;
                WORD_XOR_0; WORD_BITWISE_RULE `word_xor (word 0) x = (x:(N)word)`] THEN
    CONV_TAC(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV) THEN
    CONV_TAC(TOP_DEPTH_CONV PMUL_NORM_CONV) THEN
    REWRITE_TAC[WORD_XOR_ASSOC; WORD_XOR_0;
