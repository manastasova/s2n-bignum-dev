// Test: compare eight_blocks_aes256_gcm_preloop_tail against aws-lc EVP AES-256-GCM
//
// Build:
//   gcc -O2 -o test_eight_blocks_preloop_tail \
//     test_eight_blocks_preloop_tail.c \
//     eight_blocks_aes256_gcm_preloop_tail.S \
//     -I/home/ubuntu/auto_proofs/aws-lc/include \
//     -L/home/ubuntu/aws-lc-all/aws-lc/build/crypto \
//     -lcrypto -lpthread -ldl
//
// Run:
//   LD_LIBRARY_PATH=/home/ubuntu/aws-lc-all/aws-lc/build/crypto ./test_eight_blocks_preloop_tail
...
extern size_t eight_blocks_aes256_gcm_preloop_tail(
...
static int run_test(const char *name,
    ...
    // ----------------------------------------------------------------
    // 2) Our assembly: eight_blocks_aes256_gcm_preloop_tail
    // ----------------------------------------------------------------
    ...
    size_t ret = eight_blocks_aes256_gcm_preloop_tail(
...
int main(void) {
...

    {
        ...
        all_ok &= run_test("Test 2: NIST-like (8 blocks)", key, nonce, pt);
    }

    {
        ...
        uint8_t pt[NBYTES];
        const char *msg = "Hello, World! This is a eight-block AES-GCM test vector.....!!!";
        memset(pt, 0, NBYTES);
        memcpy(pt, msg, strlen(msg) < NBYTES ? strlen(msg) : NBYTES);
        all_ok &= run_test("Test 3: text (8 blocks)", key, nonce, pt);
    }

    {
        ...
        all_ok &= run_test("Test 4: all 0xFF (8 blocks)", key, nonce, pt);
    }
