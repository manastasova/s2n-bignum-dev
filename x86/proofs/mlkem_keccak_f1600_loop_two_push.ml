...
(* When there is no table *)
 (**** print_literal_from_elf "x86/mlkem/mlkem_keccak_f1600_loop_two_push.o";;
 ****)

let mlkem_keccak_f1600_mc_loop_push_13 = define_assert_from_elf
  "mlkem_keccak_f1600_mc_loop_push_13" "x86/mlkem/mlkem_keccak_f1600_loop_two_push.o" 
[
  0x53;                    (* PUSH (% rbx) *)
  0x55;                    (* PUSH (% rbp) *)
  0x41; 0x54;              (* PUSH (% r12) *)
 ...
 let MLKEM_KECCAK_F1600_EXEC_loop_push_13 = X86_MK_EXEC_RULE mlkem_keccak_f1600_mc_loop_push_13;;

  let MLKEM_KECCAK_F1600_SPEC = prove(
  `forall pc:num stackpointer:int64 returnaddress bitstate table.
  nonoverlapping_modulo (2 EXP 64) (pc,0x57) (val (word_sub stackpointer (word 256)), 264) /\
  nonoverlapping_modulo (2 EXP 64) (val (word_add (bitstate) (word 0):int64),200) (val (word_sub stackpointer (word 248)), 256) /\
  nonoverlapping_modulo (2 EXP 64) (pc, 0x57) (val (word_add (bitstate) (word 0):int64),200)  /\ 
  nonoverlapping_modulo (2 EXP 64) (pc, 0x57) (val (word_add (table) (word 0):int64),192)
      ==> ensures x86
  // Precondition
  (\s. bytes_loaded s (word pc) mlkem_keccak_f1600_mc_loop_push_13 /\
       read RIP s = word pc /\
       read RDI s = bitstate /\
       read RSI s = table /\
  ...
  REPEAT STRIP_TAC THEN
    ...
    `0` (* loop_two_push begin number *)
    `24` (* loop_two_push end number *)
    `pc + 0x28` (* loop body start PC *)
    `pc + 0x34` (* loop backedge branch PC -- including the jmp *)
    ...
    [
      (* loop_two_push begin < loop_two_push end *)
      ARITH_TAC;
      ...
      X86_SIM_TAC MLKEM_KECCAK_F1600_EXEC_loop_push_13 (1--12);

      (* the body of the loop *)
      REPEAT STRIP_TAC THEN
      X86_SIM_TAC MLKEM_KECCAK_F1600_EXEC_loop_push_13 (1--4) THEN
      REPEAT CONJ_TAC THENL 
      [
        CONV_TAC WORD_RULE;
      ...
      REPEAT STRIP_TAC THEN
      X86_SIM_TAC MLKEM_KECCAK_F1600_EXEC_loop_push_13 [1];
